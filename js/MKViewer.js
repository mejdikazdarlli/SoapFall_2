...
export class CARviewer {
   ...
   async initScene() {
        ...
        //this.camera = new THREE.OrthographicCamera( -3, 3, 3,-3, -5, 5 );
        ...
        await LoadModel('crowd.glb', this.scene) 
        await loadEnvironmentHDR(this.renderer, this.scene, 'default.hdr');
        this.orbit = new OrbitControls(this.camera, this.renderer.domElement);
        this.orbit.minDistance=3.8;
        this.orbit.maxDistance=5.1;
        ...
        this.orbit.addEventListener( 'change', function(camera,target){
            let val = 1.5
            target.z = 0.2
            if (target.x < -val){
                document.querySelector('.leftArrw').style.display="flex"
                document.querySelector('.RightArrw').style.display="none"
                target.x = -val;
                camera.position.x = -val;
            } else if (target.x > val){
                document.querySelector('.RightArrw').style.display="flex"
                document.querySelector('.leftArrw').style.display="none"
                target.x = val;
                camera.position.x = val;
            }
        }.bind(this, this.camera, this.orbit.target));
