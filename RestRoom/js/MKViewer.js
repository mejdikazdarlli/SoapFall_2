...
export class CARviewer {
   ...
   async initScene() {
        ...
        this.camera.position.set(-0.35,1,-5)
        this.camera.zoom = 1;
        ...
        await LoadModel('RestRoom.glb', this.scene) 
        await loadEnvironmentHDR(this.renderer, this.scene, 'env.hdr');
        this.orbit = new OrbitControls(this.camera, this.renderer.domElement);
        this.orbit.minDistance=-3.0;
        this.orbit.maxDistance=1.0;
        this.orbit.maxPolarAngle = Math.PI/2.3
...
export async function LoadModel(model, _thisScene) {
    ...
    return loader.load('asset/' + model, function (object) {
        ...
        clips.forEach(clip => {
                ...
                if(action._clip.name==="SaopdropingAction"){action.stop()}
                else
                {
                    action.play()
                }
        ...
        SceneGLB.traverse(async function (child) {
            if (child.name ==="Saopdroping"){child.material.transparent = true;child.material.opacity = 0}
        })
